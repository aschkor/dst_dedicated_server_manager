...
def add(cluster: Cluster):
    ...
    wg.remote_install(cluster.master.world_settings, Path(name), True)
    ...
    wg.remote_install(cluster.cave.world_settings, Path(name), False)

def clusters() -> list[Cluster]:
    res = []
    for file in ssh.list_files(constant.CLUSTER_DIRECTORY):
        master = Server(cf.Server(file), wg.remote_load(Path(file),True, True), [])
        cave = None
        if ssh.is_exist(constant.CLUSTER_DIRECTORY / constant.CAVE_NAME_DIR):
            cave = Server(cf.Server(file, False), wg.remote_load(Path(file),True, True), [])
        res.append(Cluster(cf.Cluster(file),tk.Token(cluster_name = file), master, cave))
    return res

def __update() -> None:
    ssh.execute(constant.CMD_UPDATE)

def __update_mods(cluster_name: str) -> None:
    ssh.execute(constant.CMD_INSTALL_MODS.format(cluster_name))
